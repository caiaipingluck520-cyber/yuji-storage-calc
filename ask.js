...
(function () {
  ...
  // ---------- 平面图：上传 → 标定比例 → 框选柜位 → 生成提问 ----------
  // 说明：所有量取都在浏览器本地完成（不上传服务器）；图上量到的是"长"和"深"，
  //       "高"平面图里没有，取左侧柜体总高（可改）。
  ...
  function mountPlan() {
    var upBtn = document.getElementById('planUploadBtn'), file = document.getElementById('planFile');
    var stage = document.getElementById('planStage'), img = document.getElementById('planImg');
    var cv = document.getElementById('planCanvas'), readout = document.getElementById('planReadout');
    var status = document.getElementById('planStatus'), actions = document.getElementById('planActions');
    var lenI = document.getElementById('planLen'), hiI = document.getElementById('planHigh'), deepI = document.getElementById('planDeep');
    if (!upBtn || !file || !cv) return;
    var S = { scale: 0, mode: 'idle', p1: null, rect: null, drag: null };
    var ctx = cv.getContext('2d');

    function paint() {
      ctx.clearRect(0, 0, cv.width, cv.height);
      if (S.p1) { ctx.fillStyle = '#c0392b'; ctx.beginPath(); ctx.arc(S.p1.x, S.p1.y, 3, 0, 6.3); ctx.fill(); }
      if (S.p2) {
        ctx.strokeStyle = '#c0392b'; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(S.p1.x, S.p1.y); ctx.lineTo(S.p2.x, S.p2.y); ctx.stroke();
        ctx.beginPath(); ctx.arc(S.p2.x, S.p2.y, 3, 0, 6.3); ctx.fill();
      }
      var r = S.drag || S.rect;
      if (r) {
        ctx.strokeStyle = '#2f7d4f'; ctx.lineWidth = 2; ctx.setLineDash([5, 4]);
    ...
    function showImage(src) {
      img.onload = function () {
        stage.hidden = false;
        S.scale = 0; S.p1 = null; S.p2 = null; S.rect = null; S.drag = null; S.mode = 'idle';
        actions.hidden = true;
        fitCanvas();
        readout.innerHTML = '① 点「标定比例」，在图上点一段<b>已知长度</b>的墙的两端（例如图纸标注 4200 的墙）。';
      };
      img.src = src;
    }
    function localPt(e) {
    ...
    window.addEventListener('resize', function () { if (!stage.hidden) fitCanvas(); });

    file.addEventListener('change', function () {
      var f = file.files && file.files[0]; if (!f) return;
      if (/pdf/i.test(f.type) || /\.pdf$/i.test(f.name)) {
        status.textContent = 'PDF 解析中（需联网取 pdf.js；离线请改传 JPG/PNG）…';
        loadPdfJs(function (lib) {
          if (!lib) { status.textContent = '离线读不了 PDF：请把图纸导出或截图成 JPG/PNG 再上传。'; return; }
          var fr = new FileReader();
          fr.onload = function () {
            lib.getDocument({ data: new Uint8Array(fr.result) }).promise
              ...
              .then(function (page) {
                ...
                return page.render({ canvasContext: c.getContext('2d'), viewport: vp }).promise.then(function () {
                  showImage(c.toDataURL('image/png'));
                  status.textContent = 'PDF 第 1 页已载入（矢量图按 2 倍清晰度渲染）。';
                });
              })
              .catch(function () { status.textContent = 'PDF 解析失败：请改用 JPG/PNG，或用 CAD 导出图片。'; });
          };
      ...
      fr2.onload = function () { showImage(fr2.result); status.textContent = '图纸已载入：' + f.name; };
      fr2.readAsDataURL(f);
    });

    document.getElementById('planCalBtn').addEventListener('click', function () {
      if (stage.hidden) { readout.textContent = '先上传一张平面图。'; return; }
      S.mode = 'calibrate'; S.p1 = null; S.p2 = null; paint();
      readout.innerHTML = '在图上点这段墙的<b>两端</b>（先点一端，再点另一端）。';
    });
    document.getElementById('planMarkBtn').addEventListener('click', function () {
      if (stage.hidden) { readout.textContent = '先上传一张平面图。'; return; }
      if (!S.scale) { readout.innerHTML = '先做①标定比例，否则量出来的尺寸不准。'; return; }
      S.mode = 'mark';
      readout.innerHTML = '按住鼠标在图上拖出柜位：<b>沿墙那一边＝长</b>，垂直方向＝深。';
    });
    document.getElementById('planResetBtn').addEventListener('click', function () {
      S.scale = 0; S.p1 = null; S.p2 = null; S.rect = null; S.drag = null; S.mode = 'idle';
      actions.hidden = true; paint();
      readout.innerHTML = '已清除量取。① 重新标定比例 ② 框选柜位。';
    });

    cv.addEventListener('mousedown', function (e) {
      var p = localPt(e);
      if (S.mode === 'calibrate') {
        if (!S.p1) { S.p1 = p; paint(); readout.innerHTML = '再点这段墙的<b>另一端</b>。'; }
        else {
          S.p2 = p; paint();
          var d = Math.round(Math.sqrt(Math.pow(S.p2.x - S.p1.x, 2) + Math.pow(S.p2.y - S.p1.y, 2)));
          readout.innerHTML = '两点间距 <b>' + d + ' px</b>。这段墙实际多少 mm？<input id="planCalMm" type="number" step="10" placeholder="如 4200">'
            + '<button type="button" class="secondary-btn" id="planCalOk" style="min-height:30px;padding:0 10px">确认比例</button>';
          var ok = document.getElementById('planCalOk');
          if (ok) ok.addEventListener('click', function () {
            var mm = parseFloat((document.getElementById('planCalMm') || {}).value);
            if (!mm || mm <= 0) { readout.innerHTML = '请填这段墙的实际长度（mm）。'; return; }
            S.scale = mm / d;
            readout.innerHTML = '比例已定：<b>1 px ≈ ' + S.scale.toFixed(2) + ' mm</b>（' + d + ' px = ' + Math.round(mm) + ' mm）。'
              + '② 点「框选柜位」，在图上拖出柜子位置。';
            S.mode = 'idle';
          });
        }
      ...
      if (S.mode === 'mark') { S.drag = { x0: p.x, y0: p.y, x1: p.x, y1: p.y }; paint(); }
    });
    cv.addEventListener('mousemove', function (e) {
      if (!S.drag) return;
      var p = localPt(e); S.drag.x1 = p.x; S.drag.y1 = p.y; paint();
    });
    cv.addEventListener('mouseup', function () {
      if (!S.drag) return;
      var r = S.drag; S.drag = null;
      var wpx = Math.abs(r.x1 - r.x0), hpx = Math.abs(r.y1 - r.y0);
      if (wpx < 4 || hpx < 4) { S.rect = null; paint(); readout.textContent = '框选太小了，重拖一次。'; return; }
      S.rect = r; paint();
      var L = Math.round(wpx * S.scale), D2 = Math.round(hpx * S.scale);
      var H = Math.round(parseFloat((document.getElementById('generalHeight') || {}).value) || 2200);
      lenI.value = L; deepI.value = D2; hiI.value = H;
      actions.hidden = false;
      readout.innerHTML = '框选结果：<b>长 ' + L + ' mm × 深 ' + D2 + ' mm</b>（图上量取）；高 ' + H + ' mm 取左侧柜体总高，可改。'
        + '确认后点「按这个尺寸提问」。';
    });

    document.getElementById('planAskBtn').addEventListener('click', function () {
      var L = Math.round(parseFloat(lenI.value) || 0), H = Math.round(parseFloat(hiI.value) || 0), Dp = Math.round(parseFloat(deepI.value) || 0);
      if (!L || !H || !Dp) { readout.textContent = '长/高/深都要有数字。'; return; }
      var scene = (window.YujiGeneral && window.YujiGeneral.scene) ? window.YujiGeneral.scene() : '';
      var q = (scene || '') + '柜 长' + L + ' 高' + H + ' 深' + Dp;
  ...
  if (document.addEventListener) document.addEventListener('DOMContentLoaded', function () { setTimeout(function () { mount(); mountPlan(); }, 60); });
})();
