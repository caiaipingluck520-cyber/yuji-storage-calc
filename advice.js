 ...
 * ------------------------------------------------------------------
 * 数据源：
 *  ① 近藤典子《尺寸间的井井有条——近藤典子手绘图鉴》中国建筑工业出版社 2016
 *     （三档进深 20/30/40cm、人体比例尺、就近/集中/指定座位、透明容器、三排可视上限）
 *  ② 近藤典子《助你打造一个井井有条的家》中国建筑工业出版社 2013
 *     （物尽其用·规模适当、家务动线就近衔接、衣帽间上装 60/下装 40 分开挂、长期规划）
 *  ③ 达哥量化收纳体系 + 适老镜F（中国院适老建筑实验室）
 * 读札记：E:\蒸馏工作区\产物\读札记\读札记3_近藤典子_尺寸间的井井有条.md / 读札记4
 * ------------------------------------------------------------------
...
(function () {
  ...
  function sceneAdvice(r) {
    ...
    if (sc === '衣帽' || (cat === '衣物' && r.method === 'hang')) {
      out.push('上装挂宽 60cm、下装挂宽 40cm（近藤典子）——上装比下装宽，两者必须分开挂，否则宽的上装会遮住下装、白白吃掉一排位置。');
  ...
  function buildStorageAdvice(r, opts) {
    ...
    var g1 = [];
    if (!fit) g1.push('物品进深 ' + round(item.d) + 'mm 超出近藤典子的常规三档（20/30/40cm，微波炉 50cm）——按"大件另算"处理：大件单独给格，不要为了它把整排柜体加深。');
    if (fit || spareDepth >= 150) {
      if (fit) g1.push('物品本身进深 ' + round(item.d) + 'mm，对应近藤典子「' + DEPTH_NAME[fit] + '」。');
      if (spareDepth >= 150) {
    ...
    if (b) {
      ...
      if (b.kind === 'hang') {
        ...
        if (b.low.length) g2.push('第 ' + b.low[0].i + '–' + b.low[b.low.length - 1].i + ' 层（顶面低于垂手指尖 ' + round(bd.hand) + 'mm）放重物：米油、锅具、箱装囤货，符合适老"重物低放"，起身也不费腰。');
    ...
    g3.push('三条收纳铁律（近藤典子）：**就近存放**——东西放在它被使用的地方，不是放在空墙处；**同物集中**——同类只放一处，不分散到三个柜子；**指定座位**——每样东西有固定位置并贴标签，用完自动归位，不会"反弹"。');
    ...
    g5.push('近藤典子「物尽其用、规模适当」：收纳空间不是越大越好——太大就会什么都往里塞，取用反而更不方便。' + (r.capacity ? '当前理论容量 ' + r.capacity + ' ' + unit + '、建议实际放 ' + r.safe + ' ' + unit + '，' : '') + '请按家里实际物品数量定柜，不要为了"看起来能装"而加深加高。');
  ...
  function adviceToText(r, opts) {
    ...
    return head + body + '\n\n依据：近藤典子《尺寸间的井井有条》《井井有条的家》+ 达哥量化收纳 + 适老镜F（中国院适老建筑实验室）\n';
  }
