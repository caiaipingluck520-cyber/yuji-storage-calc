...
(function () {
  ...
  function render() {
    ...
    out += page(
      '<h2>一、核算口径声明</h2>' +
      '<ul class="rpt-list">' +
        '<li><b>物品尺寸</b>：《万物与尺度》实测图鉴（鞋宽 女80/男100、客拖115、雨靴鞋面158 mm 等）。</li>' +
        '<li><b>核算方法</b>：达哥量化收纳口径（每米挂杆、每层可放、百纳箱换算）+ 近藤典子尺寸学（三档进深 20/30/40cm、人体比例尺）。</li>' +
        '<li><b>鞋类口径</b>：' + (d.shoeLay === 'side' ? '并排直放（保守，每米约 5 双）' : '错位摆放（达哥口径，每米约 8 双男鞋）') + '。</li>' +
  ...
  function buildDeck(d) {
    ...
    (function () {
        ...
        p2.addText([
          { text: '物品尺寸：《万物与尺度》实测图鉴（鞋宽 女80 / 男100、客拖115、雨靴鞋面158 mm 等）\n' },
          { text: '核算方法：达哥量化收纳口径（每米挂杆、每层可放、百纳箱换算）+ 近藤典子尺寸学（三档进深 20/30/40cm、人体比例尺）\n' },
          { text: '鞋类口径：' + (d.shoeLay === 'side' ? '并排直放（保守，每米约 5 双）' : '错位摆放（达哥口径，每米约 8 双男鞋）') + '\n' },
        ...
        // 建议摘要
        var p6 = pptx.addSlide(); head(p6, '六、收纳建议摘要', '近藤典子 × 达哥量化 × 适老镜F');
        p6.addText([
